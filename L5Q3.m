...
for ind = 1:length(th)
    for indF = 1:length(freq)
        ...
        B_SI = suscpetance_SIShifted(omega, dy, dz, m, W, sy);
        B_I = suscpetance_IShifted(omega, dy, dz, m, W, sy);
        ...
        %Final Matrix
        FmatTM = (matEdgeTM^2)*(matBetTM^(N-2))*(matTxTM^N);
        FmatTE = (matEdgeTE^2)*(matBetTE^(N-2))*(matTxTE^N);
